...
class LibretroWrapper:
	...
	class RetroControllerDescription(ctypes.Structure):
		_fields_ = [
			('desc', ctypes.c_char_p),
			('id', ctypes.c_uint)
		]

	# class RetroControllerInfo(ctypes.Structure):
	# 	_fields_ = [
	# 		('types', ctypes.POINTER(self.RetroControllerDescription)),
	...
	class ControllerInfo():
		""" Wrapped system info """
		def __init__(self, retro_input_descriptor=None):
			self.port = int(retro_input_descriptor.port)
			self.device = int(retro_input_descriptor.device)
			self.index = int(retro_input_descriptor.index)
			self.id = int(retro_input_descriptor.id)
			self.description = xstr(retro_input_descriptor.description)

		def __getitem__(self, item):
			return getattr(self, item)

		def __repr__(self):
			return '(port={}, device={}, index={}, id={},' \
				   ' description={})'.format(
					   self.port, self.device, self.index,
	...
	def __init__(self, library_path):
		...
		retro_input_descriptor = lib.retro_set_controller_port_device
		retro_get_system_info.argtypes = \
			[ctypes.POINTER(self.RetroSystemInfo)]
		retro_get_system_info.restype = None

		retro_input_descriptor.argtypes = \
			[ctypes.POINTER(self.RetroInputDescriptor)]

		system_info = self.RetroSystemInfo()
		input_info = self.RetroInputDescriptor()

		retro_get_system_info(ctypes.byref(system_info))
		self.system_info = self.SystemInfo(system_info)

		retro_input_descriptor(ctypes.byref(input_info))
		self.input_info = self.ControllerInfo(input_info)
