...
def generate_info_description(info_dict_in):
	...
	description_out = info_dict_in['categories']+' for '+info_dict_in['systemname']
	if 'authors' in info_dict_in.keys() and len(info_dict_in)>0:
		description_out = description_out+' by '+info_dict_in['authors']
	if 'supported_extensions' in info_dict_in.keys() and len(info_dict_in['supported_extensions'])>0:
		description_out = description_out+'[CR]'+'Supported files: '+info_dict_in['supported_extensions']
	if 'notes' in info_dict_in.keys() and len(info_dict_in['notes'])>0:
		description_out = description_out+'[CR][CR]'+'Game Addon Notes: '+info_dict_in['notes']
	if len([x for x in info_dict_in.keys() if x.startswith('firmware') and x.endswith('_opt')])>0:
		if any([info_dict_in[y]=='false' for y in [x for x in info_dict_in.keys() if x.startswith('firmware') and x.endswith('_opt')]]):
