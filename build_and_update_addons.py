		...
		if build_data['libretro_info_dict'][ii][-1] is not None:
			...
			if core_name in build_data['dont_install_these_cores'][ii]:
				...
				if 'game.libretro.'+core_name+'_buildbot' in [os.path.split(x)[-1].rsplit('-',1)[0] for x in build_data['current_builds'][ii]]:
					...
					current_build = None
			build_data['addon_xml'][ii].append(create_addon_xml_dict(kodi_platform=platforms,addon_id='game.libretro.'+core_name+'_buildbot',addon_name=build_data['libretro_info_dict'][ii][-1]['display_name'],addon_version=current_version,addon_commit=build_data['libretro_bb_info_dict'][ii]['commit'][jj],addon_date=build_data['libretro_bb_info_dict'][ii]['date'][jj],addon_supported_extensions=build_data['libretro_info_dict'][ii][-1]['supported_extensions'],addon_supported_standalone=build_data['libretro_info_dict'][ii][-1]['supports_no_game'],addon_license=build_data['libretro_info_dict'][ii][-1]['license'],addon_description=generate_info_description(build_data['libretro_info_dict'][ii][-1]),addon_system_description=build_data['libretro_info_dict'][ii][-1]['systemname'],addon_library_key=build_data['kodi_libraries'][ii],addon_library_value=build_data['libretro_bb_info_dict'][ii]['filename'][jj].replace('.zip',''),buildbot_source_url=build_data['bb_urls'][ii]+build_data['libretro_bb_info_dict'][ii]['filename'][jj],addon_resources_data=get_addon_resources_data(core_name=core_name,resources_dir=script_settings['addon_resources_dir']),build_addon=build_this_addon,current_build=current_build))
